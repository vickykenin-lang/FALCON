...
ORGANS={"artifacts","autonomic","brain","execution","governance","interface","learning","memory","nervous_system","observability","scheduler","senses"}
        ...
        for organ in ORGANS:
            for path in (ROOT/organ).rglob("*.py"):
                ...
                for node in ast.walk(tree):
                    ...
                    for name in names:
                        target=name.split(".")[0]
                        if target in ORGANS and target!=organ: violations.append(f"{path.relative_to(ROOT)} imports {name}")
        self.assertEqual(violations,[],"Cross-organ implementation imports:\n"+"\n".join(violations))
    def test_composition_root_may_wire_organs(self): self.assertTrue((ROOT/"bootstrap.py").exists())
