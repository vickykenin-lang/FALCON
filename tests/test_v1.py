...
class FalconV1Tests(unittest.TestCase):
    ...
    def action(self,mission): return Event("ACTION","brain",{"adapter":"fake","operation":"do_work","capability":CAP,"args":{},"risk":"low"},correlation_id=mission.mission_id)
    ...
    def test_autonomous_loop_success(self):
        with tempfile.TemporaryDirectory() as d:
            r=self.runtime(d); m=r.accept("execute automatically"); ex=Executor(); adapter=FakeAdapter(); ex.register(adapter); self.assertEqual(AutonomousLoop(r,ex,self.governance()).run(m,self.action(m)).status,"SUCCEEDED")
    def test_autonomous_loop_adapts_and_retries(self):
        with tempfile.TemporaryDirectory() as d:
            r=self.runtime(d); m=r.accept("recover automatically"); ex=Executor(); adapter=FakeAdapter(1); ex.register(adapter); result=AutonomousLoop(r,ex,self.governance(),max_attempts=3).run(m,self.action(m)); self.assertEqual(result.status,"SUCCEEDED"); self.assertEqual(adapter.calls,2)
    def test_governance_fails_closed_without_capability(self):
        allowed,reason=self.governance().authorize(Event("ACTION","brain",{"adapter":"fake","operation":"do_work","risk":"low"})); self.assertFalse(allowed); self.assertEqual(reason,"capability_required")
    def test_governance_blocks_unlisted_capability(self):
