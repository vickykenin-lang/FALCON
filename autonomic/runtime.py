...
Brain, Memory and Nervous System implementations are wired at the composition
root, so each organ can be replaced independently.
"""
import json
...
class Runtime:
    ...
    def accept(self, objective: str, source: str = "founder", source_id: str | None = None) -> Mission:
        mission=Mission(objective=objective); mission.transition("DISCOVERING")
        ...
        self.bus.publish(self.brain.understand(mission)); self.checkpoint(mission); return mission
    ...
    def advance(self, mission: Mission, result: Event | None = None) -> Mission:
        ...
        if mission.status in transitions: mission.transition(transitions[mission.status])
            ...
            ok=bool(result and result.payload.get("ok")); mission.transition("SUCCEEDED" if ok else "ADAPTING")
        elif mission.status=="ADAPTING":
            mission.attempts+=1; mission.transition("PLANNING")
        ...
        self.checkpoint(mission); return mission

    def checkpoint(self, mission: Mission) -> None:
        p=self.state_dir/"checkpoints"; p.mkdir(parents=True,exist_ok=True)
