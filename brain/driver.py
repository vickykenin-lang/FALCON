"""Brain-driven autonomous mission coordinator with memory-grounded planning."""
from contracts.models import Event, Mission


class BrainDriver:
    def __init__(self, brain, executor, governance, runtime, memory=None, max_replans: int = 3):
        self.brain = brain
    ...
    def _context(self, mission: Mission, supplied: dict | None) -> dict:
        context=dict(mission.context or {})
        context.update(supplied or {})
        if self.memory is not None:
            context.update(self.memory.context_for(mission.objective))
        return context

    def run(self, mission: Mission, context: dict | None = None) -> Mission:
        working_context=self._context(mission,context)
        replans=0
        while replans <= self.max_replans:
            ...
            if plan_event.event_type=="FAILURE":
                mission.transition("BLOCKED"); self.runtime.checkpoint(mission); return mission
            ...
            if plan.get("needs_more_context"):
                ...
                self.runtime.checkpoint(mission); return mission
            ...
            replans+=1
            working_context.update({"previous_plan":plan,"previous_evidence":evidence,"replan_attempt":replans})
            if self.memory is not None: working_context.update(self.memory.context_for(mission.objective))
            if replans>self.max_replans:
                mission.transition("FAILED"); self.runtime.checkpoint(mission); return mission
