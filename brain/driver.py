"""Brain-driven autonomous mission coordinator with enforced mission control."""
from contracts.models import Event, Mission
class BrainDriver:
    def __init__(self,brain,executor,governance,runtime,memory=None,max_replans:int=3,control=None):
        self.brain=brain; self.executor=executor; self.governance=governance; self.runtime=runtime; self.memory=memory; self.max_replans=max_replans; self.control=control
    def _context(self,mission:Mission,supplied:dict|None)->dict:
    ...
    def run(self,mission:Mission,context:dict|None=None)->Mission:
        ...
        while replans<=self.max_replans:
            ...
            evidence=[]; all_ok=True; actions=self.brain.action_events(mission,plan_event)
            if not actions: all_ok=False; evidence.append({"ok":False,"error":"empty_plan"})
            for action in actions:
                if not self._control_gate(mission): return mission
                allowed,reason=self.governance.authorize(action)
                if not allowed:
                    mission.transition("BLOCKED"); self.runtime.bus.publish(Event("ALERT","governance",{"mission_id":mission.mission_id,"reason":reason},target="interface",correlation_id=mission.mission_id)); self.runtime.checkpoint(mission); return mission
                if not self._control_gate(mission): return mission
                result=self.executor.execute(action); self.runtime.bus.publish(result); evidence.append(result.payload)
                if not result.payload.get("ok"): all_ok=False; break
            if mission.status=="EXECUTING": self.runtime.advance(mission)
            verification=Event("RESULT","brain_driver",{"ok":all_ok,"evidence":evidence,"success_criteria":plan.get("success_criteria",[])},target="autonomic",correlation_id=mission.mission_id); self.runtime.bus.publish(verification)
            if mission.status=="VERIFYING": self.runtime.advance(mission,verification)
            if mission.status=="SUCCEEDED": return mission
            replans+=1; working_context.update({"previous_plan":plan,"previous_evidence":evidence,"replan_attempt":replans})
