...
def self_test(runtime: Runtime) -> int:
    """Minimal executable health verification used by CI and operators."""
...
def main() -> int:
    ...
    if a.self_test:
        return self_test(r)
    if not a.command:
        p.error("command is required unless --self-test is used")
    if a.command == "health":
        print(json.dumps(r.heartbeat().to_dict(), indent=2))
        return 0
    if not a.text:
        p.error("mission requires an objective")
    m = r.accept(" ".join(a.text))
