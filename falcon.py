...
def build_service(state_dir:str=".falcon",tick_seconds:float=1.0,heartbeat_seconds:float=30.0):
    runtime=build_runtime(state_dir=state_dir); bridge=SchedulerBridge(runtime.bus.publish,runtime.accept); scheduler=Scheduler(f"{state_dir}/schedules.json",on_due=bridge.on_due)
    return FalconService(runtime,scheduler,tick_seconds=tick_seconds,heartbeat_seconds=heartbeat_seconds)

def main()->int:
    p=argparse.ArgumentParser(prog="falcon"); p.add_argument("--self-test",action="store_true"); p.add_argument("--state-dir",default=".falcon"); p.add_argument("--tick-seconds",type=float,default=1.0); p.add_argument("--heartbeat-seconds",type=float,default=30.0); p.add_argument("--host",default="0.0.0.0"); p.add_argument("--port",type=int,default=8080); p.add_argument("command",nargs="?",choices=["health","mission","serve","dashboard"]); p.add_argument("text",nargs="*"); a=p.parse_args(); r=build_runtime(state_dir=a.state_dir)
