...
def main()->int:
    p=argparse.ArgumentParser(prog="falcon"); p.add_argument("--self-test",action="store_true"); p.add_argument("--state-dir",default=".falcon"); p.add_argument("--tick-seconds",type=float,default=1.0); p.add_argument("--heartbeat-seconds",type=float,default=30.0); p.add_argument("--host",default="0.0.0.0"); p.add_argument("--port",type=int,default=8080); p.add_argument("command",nargs="?",choices=["health","mission","serve","dashboard","telegram"]); p.add_argument("text",nargs="*"); a=p.parse_args()
    ...
    if a.command=="telegram":run_telegram(r); return 0
